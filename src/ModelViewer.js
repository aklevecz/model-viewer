...
export class ModelViewer {
    ...
    setupEventListeners() {
        ...
        const updateCountInputs = [
            'top-view-enabled', 'middle-view-enabled', 'bottom-view-enabled',
            'angle-top', 'angle-middle', 'angle-bottom',
            'dist-close', 'dist-medium', 'dist-far',
            'top-rotation', 'middle-rotation', 'bottom-rotation',
            'top-angle', 'middle-angle', 'bottom-angle'
        ];
        ...
        const captureImage = (viewConfig, viewIndex, distanceObj) => {
            ...
            return new Promise(resolve => {
                ...
                if (viewConfig.count > 1) {
                    ...
                    console.log(`Image ${viewIndex + 1}/${viewConfig.count}: Center ${viewConfig.centerRotation.toFixed(1)}° + offset ${angleOffset.toFixed(1)}° = ${rotation.toFixed(1)}°`);
                } else {
                ...
                fetch(dataUrl)
                    ...
                    .then(blob => {
                        ...
                        const offsetText = viewConfig.count > 1 ?
                            ((viewIndex === 0) ? "start" :
                                (viewIndex === viewConfig.count - 1) ? "end" :
                                    `${viewIndex}`
                            ) : "center";
